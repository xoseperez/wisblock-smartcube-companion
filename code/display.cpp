...
static void display_draw_bmp(const GUI_BITMAP *bmp , uint8_t x, uint8_t y) {
    ...
    _display_screen.startWrite();
    _display_screen.setAddrWindow(x, y, bmp->xSize, bmp->ySize);

    for ( uint64_t i = 0 ; i < bufSize ; i++ ) {
        if(color == bmp->date[i]) {
            count++;
        } else {
            _display_screen.writeColor(color, count); 
            count = 1;
            color = bmp->date[i];
        }
    }
...
void display_update_cube() {
    ...
    for (uint8_t i=0; i<3; i++) for (uint8_t j=0; j<3; j++) {
      _display_screen.fillRect(x + j * block, y + i * block, size, size, colors[cc[index++]]);
    }
    ...
    y = offset_y + 3 * block;
    for (uint8_t i=0; i<3; i++) for (uint8_t j=0; j<3; j++) {
      _display_screen.fillRect(x + i * block, y + (2-j) * block, size, size, colors[cc[index++]]);
    }
    ...
    y = offset_y + 3 * block;
    for (uint8_t i=0; i<3; i++) for (uint8_t j=0; j<3; j++) {
      _display_screen.fillRect(x + (2-j) * block, y + (2-i) * block, size, size, colors[cc[index++]]);
    }
    ...
    y = offset_y + 3 * block;
    for (uint8_t i=0; i<3; i++) for (uint8_t j=0; j<3; j++) {
      _display_screen.fillRect(x + (2-i) * block, y + j * block, size, size, colors[cc[index++]]);
    }
...
void display_text(char * text, uint16_t x, uint16_t y, uint8_t align) {
    ...
    int16_t a, b;
    uint16_t w, h;
    _display_screen.getTextBounds(text, 0, 0, &a, &b, &w, &h);
    ...
    // Display
    _display_screen.setCursor(x, y);
    _display_screen.println(text);    
...
void display_battery() {
    ...
    if (battery > 25) {
        _display_screen.setTextColor(ST77XX_WHITE, ST77XX_BLACK);
    } else {
        _display_screen.setTextColor(ST77XX_RED, ST77XX_BLACK);
    }
    _display_screen.setTextSize(1);
...
void display_show_intro() {
    ...
    _display_screen.setTextColor(ST77XX_GREEN, ST77XX_BLACK);
...
void display_hide_timer() {
    _display_screen.fillRect(160, 180, 150, 40, ST77XX_BLACK);
}
...
void display_show_timer() {
    ...
    _display_screen.setTextColor((3 == status) ? ST77XX_WHITE : ST77XX_GREEN, ST77XX_BLACK);
    _display_screen.setTextSize(2);
